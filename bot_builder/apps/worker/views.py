...
def user_photo_upload(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return JsonResponse({"error": "Пользователь с таким tg_id не найден"}, status=404)
        ...
        if form.is_valid():
            ...
            # Если данные "первого запроса" не заполнены, заполняем их
            if not user.summa_first:
            ...
            # Формируем данные для отправки задачи
            ...
            try:
                # Отправляем запрос с фото на сервер
                ...
                print(f"Ошибка при отправке задачи на сервер: {e}")
                send_status_message(user.tg_id, message='Произошла ошибка при отправке задачи. Попробуйте снова.')
    ...
    # Рендерим страницу для загрузки фото
    response_render = render(request, 'photo_input.html', {
        'form': form,
        'tg_id': tg_id,
        'close': False,
