...
#                 for group_id in list(media_groups.keys()):
#                     if (current_time - media_group_timestamps.get(group_id, 0) > 6 and 
...
#         def get_best_photo(photos):
#             """Выбирает фото наилучшего качества"""
#             if not photos:
#                 return None
#             # Сортируем по высоте и ширине (площади изображения)
#             return max(photos, key=lambda x: (x.get('width', 0) * x.get('height', 0)))
...
#             # Собираем фото наилучшего качества из всех сообщений группы
#             for msg in messages[:10]:
#                 if 'photo' in msg:
#                     best_photo = get_best_photo(msg['photo'])
#                     if best_photo:
...
#                 print(f"Создано событие для группы {group_id} с {len(photos)} фото в максимальном качестве")
#                 return True
#             return False
...
#                         # Если есть отдельное фото (не в группе), берём его в максимальном качестве
#                         if message.get("photo") and not media_group_id:
#                             best_photo = get_best_photo(message["photo"])
#                             if best_photo:
#                                 message["photo"] = [best_photo]
